...
const Managers: Array<{
  ...
  { key: "forge", name: "Forge", description: "Fitness planning and recovery." },
  { key: "olive", name: "Olive", description: "Nutrition and health guidance." },
  { key: "lexicon", name: "Lexicon", description: "Reading, learning, and discussion." },
];
  ...
  async function onSend() {
    ...
    try {
      ...
      if (
        ...
        setMessages((prev) =>
          prev.map((m) => (m.id === optimisticId ? (savedUser as ConversationRow) : m))
        );
      }
        ...
        <div className="flex min-h-[520px] flex-col rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-black">
          ...
          >
            {loading ? (
              ...
              <div className="text-sm text-zinc-600 dark:text-zinc-400">
                Start a conversation with {Managers.find((m) => m.key === activeManager)?.name}.
              </div>
            ) : (
